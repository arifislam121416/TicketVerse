...
import { FaTicketAlt,FaBars, FaTimes, FaUser, FaSignOutAlt, FaThLarge } from "react-icons/fa";
...
if (isPending) {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b border-white/5 bg-slate-950/90 backdrop-blur-md py-3.5 px-4 md:px-6">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="hidden sm:flex items-center gap-8">
          ...
          <Link
            href="/ticket"
            className={`text-sm font-medium transition-colors ${pathname.startsWith("/ticket") ? "text-pink-500 font-semibold" : "text-slate-300 hover:text-white"}`}
          >
            Browse Ticket
          </Link>
          <Link
            href="/tricket"
            className={`text-sm font-medium transition-colors ${pathname.startsWith("/tricket") ? "text-pink-500 font-semibold" : "text-slate-300 hover:text-white"}`}
          >
           Tricket
          </Link>
          {isLoggedIn && (
...
/>
 ...
 <button
  ...
  disabled={loading}
  className="w-full flex items-center gap-2.5 px-4 py-2.5 text-left text-xs font-semibold text-red-400 hover:bg-red-500/5"
>
